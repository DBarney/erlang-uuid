...
uuid4() ->
    ...
    U2 = random:uniform((2 bsl 12) - 1),
    U3 = random:uniform((2 bsl 32) - 1),
    U4 = random:uniform((2 bsl 30) - 1),

    uuid4(U0, U1, U2, U3, U4).


%% @private
%% @doc  Create a 128 bit binary (UUID v4) from input
-spec uuid4(U0::integer(), U1::integer(), U2::integer(), U3::integer(),
            U4::integer()) -> uuid().
uuid4(U0, U1, U2, U3, U4) ->
...
filter_if({IfName, IfConfig}) ->
    case IfName of
        ...
        "lo" -> false;
        _ -> lists:keymember(hwaddr,1,IfConfig)
    end.
